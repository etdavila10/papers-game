  ...
  return (
    <div>
      ...
      <div  className="article-containers">
        <div  className={props.newerArticle === 1 ? "article-entry new" : "article-entry"} onClick={() => {props.openArticle(props.article1.id["_text"])}} >
          <h2>Title: {props.article1.title["_text"]}</h2>
          <h2>Date: {props.article1.published["_text"].substring(0,10)}</h2>
        </div>
        <div className={props.newerArticle === 1 ? "article-entry" : "article-entry new"} onClick={() => {props.openArticle(props.article2.id["_text"])}}>
